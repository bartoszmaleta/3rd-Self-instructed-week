...
def display_inventory_with_columns():
...
display_inventory_with_columns()
...
sorted_inv = sorted(inv.items(), key=operator.itemgetter(-1), reverse=True)
display_inventory_with_columns()
print('\n-------------------------------------------- Step 3.3.')

longest_string = max(len(line) for line in inv)
...
def display_inventory_with_columns_desc(inventory, order):
    if order == 'count,desc':
...
desc_sorted_inv = sorted(inv.items(), key=operator.itemgetter(-1))
display_inventory_with_columns_desc(inv, 'count,desc')
...
asc_sorted_inv = sorted(inv.items(), key=operator.itemgetter(-1), reverse=True)
display_inventory_with_columns_desc(inv, 'count,asc')
